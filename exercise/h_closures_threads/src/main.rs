...
fn main() {
    ...
    // Note: expensive_sum() parameter takes a vector by value so we specify `move` for the closure
    // to be a move closure, which takes ownership of the vector.
    ...
    //
    let sum = handle.join().expect("error retrieving result");
